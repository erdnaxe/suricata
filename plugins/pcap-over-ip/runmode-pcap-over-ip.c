...
#define PCAP_OVER_IP_BUFFER_SIZE_MAX     67108864U // 64 MiB

const char *RunModeIdsPcapOverIPGetDefaultMode(void)
{
    return "autofp";
}

void RunModeIdsPcapOverIPRegister(int slot)
{
    RunModeRegisterNewRunMode(
            slot, "single", "Single threaded Pcap-over-IP mode", RunModeIdsPcapOverIPSingle, NULL);
    RunModeRegisterNewRunMode(slot, "autofp",
            ...
            RunModeIdsPcapOverIPAutoFp, NULL);
}
...
static void *ParsePcapOverIPConfig(const char *socket_addr)
{
    ...
    if (SCConfGetInt("pcap-over-ip.buffer-size", &value) == 1) {
    ...
    if (SCConfGet("pcap-over-ip.checksum-checks", &tmpctype) == 1) {
    ...
    aconf->threads = 1;
    intmax_t threads = 0;
    if (SCConfGetInt("pcap-over-ip.threads", &threads) == 1 && threads > 0 && threads < INT_MAX) {
...
static uint16_t PcapOverIPConfigGetThreadsCount(void *conf)
{
    PcapOverIPIfaceConfig *pfp = (PcapOverIPIfaceConfig *)conf;
    return pfp->threads;
}

int RunModeIdsPcapOverIPAutoFp(void)
{
    SCEnter();
    ...
    if (SCConfGet("pcap-over-ip.socket_addr", &socket_addr) == 0 || socket_addr == NULL) {
    ...
    SCLogDebug("RunModeIdsPcapOverIPAutoFp initialised");

    return 0;
}

int RunModeIdsPcapOverIPSingle(void)
{
    SCEnter();
    ...
    if (SCConfGet("pcap-over-ip.endpoint", &endpoint) == 0 || endpoint == NULL) {
        FatalError("Failed retrieving pcap-over-ip.endpoint from configuration.");
    }
    ...
    SCLogDebug("RunModeIdsPcapOverIPSingle initialised");
